...
int main(int argc, char *argv[] ) {
    ...
    if (rank == master) {
        ...
        // Send parts of image according to temporal_size 
    ...
    if (rank != master) {
        ...
        memcpy(edge_image, section_sobel_image, (imgs_size/size) + 1);
        ...
        for(int j = 1; j < size; j++) {
            ...
            memcpy(edge_image + (j * (imgs_size/size)), section_sobel_image + (j * (imgs_size/size)), (imgs_size/size) + 1);
        }
